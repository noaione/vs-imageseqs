...
Every ordinary file is three pixels wide and two pixels tall with hand-picked
samples, so the expected planes are easy to state in the validator. The
monochrome sources are seven by five: ``mono-alpha.png`` feeds the eight-bit
HEIF and AVIF fixtures, while ``mono-10.png``, ``mono-12.png`` and
``mono-alpha-12.png`` feed the nominal-depth fixtures.

Those container fixtures are generated by hand from the PNG files this script
writes, because an encoder is the only way to produce them. Re-run these
...
``tests/readalpha.vpy``:
    ...
    heif-enc -q 100 --bit-depth 10 -p x265:lossless=1 -o tests/fixtures/mono-10.heic tests/fixtures/mono-10.png
    heif-enc -q 100 --bit-depth 12 -p x265:lossless=1 -o tests/fixtures/mono-12.heic tests/fixtures/mono-12.png
    ...
    avifenc --lossless --qalpha 100 -d 12 -o tests/fixtures/mono-alpha-12.avif tests/fixtures/mono-alpha-12.png
...
that exact profile in their PNG ``iCCP`` chunks.
...
ICC_PROFILE = base64.b64decode(
    ""
)
...
def png(
    ...
    if icc_profile is not None:
        profile = chunk(b"iCCP", b"imgseqs\0\0" + zlib.compress(icc_profile, 9))
    document = (
        b"\x89PNG\r\n\x1a\n"
        + chunk(b"IHDR", header)
        + profile
...
def main() -> None:
    ...
    with open(write("icc-srgb.icc"), "wb") as handle:
        handle.write(ICC_PROFILE)
    png(
        write("icc-rgb8.png"),
        RGB,
        8,
    ...
    # Sixteen-bit PNG inputs for the 10- and 12-bit monochrome HEIF/AVIF
    # fixtures. The encoder takes the high `depth` bits of the gray channel, so
    # left-align the deliberately chosen gray samples in the sixteen-bit source
    # word. Alpha is a normalized channel in avifenc, so scale it across the
    # full sixteen-bit range; this keeps the nominal-depth alpha values exact
    # after the encoder reduces them to twelve bits. HEIF needs grayscale-only
    # input to keep its monochrome colorspace; AVIF also gets a gray-alpha source
    # so its alpha plane is covered at twelve bits.
    for depth in (10, 12):
        maximum = (1 << depth) - 1
        rows = [
            [
        ...
        png(
            write(f"mono-{depth}.png"),
            GRAY,
            16,
            [[row[index] for index in range(0, len(row), 2)] for row in rows],
            width=MONO_WIDTH,
            height=MONO_HEIGHT,
        )
