...
`tests/readalpha.vpy` now checks, at 278 checks, alongside the ten bit avif's
...
- **a 16-bit file whose decoder does not fill the range** is a different
  ...
  touch it.
- **a ten bit monochrome heic has no fixture.** the heif row of this plan is
  implemented and unverified: `mono-alpha-10.avif` covers the monochrome depth
  for a container whose samples go through `image`, and the heic one needs
  `heif-enc`, which is not installed on the machine this was built on. a
  `heif-enc` pass over the same ten bit gray source would close it, and the
  monochrome heif path it would exercise is the one where libheif's own planes
  are handed over at the depth the handle reports.
