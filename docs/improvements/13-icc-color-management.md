...
- result: `icc_profile=False` keeps the current frame properties;
...
The source option is a boolean, not a target color-space selector:
...
`ImageInfo` stores an optional shared `Arc<[u8]>`, so the profile is retained
once during probing and can be attached to frames without reopening the source
file or copying the bytes into every internal image record.

| reader | current state | implementation |
| --- | --- | --- |
| `image` formats | `icc_profile()` is queried during probe | retain the returned profile bytes |
| HEIF/HEIC | libheif exposes raw profile data and presence | copy the selected `rICC`/`prof` profile |
| AVIF | the box walk recognizes `prof`/`rICC` | retain the primary item's profile payload |
| JPEG 2000 | the JP2 box walk recognizes ICC `colr` | retain the profile bytes after the `colr` header |
...
The source sets the property as binary data with the exact key
...
The validator now uses a small, valid sRGB profile generated by
`tests/make-alpha-fixtures.py`:
...
- `icc-rgb8.png` and `icc-rgba8.png` are compared with their independent
  `icc-srgb.icc` bytes;
- RGB and alpha frames remain pixel- and format-identical to the disabled path;
- `ReadAlpha` exposing the property without changing the alpha plane;
- the default path omitting `ICCProfile` while retaining `ImgSeqHasICC`.
